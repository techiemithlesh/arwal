...
import { tradeRoleWisePendingApi } from "../../../api/endpoints";
import axios from "axios";
import { exportToExcel, exportToPdf } from "../../../utils/exportUtils";
import { createSearchParams, Link } from "react-router-dom";

function LevelWisePending() {
  ...
  const handleExport = () => {
    const columns = [
      ...
      { key: "totalTrade", label: "Total No of Form(s)" },
    ];
        ...
        <table ref={resultRef} className="w-full text-sm table-auto">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-2 border text-left">App Wise</th>
          ...
          <tbody>
            {dataList.map((item, idx) => (
              <tr key={idx} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border">{item.totalTrade}</td>
                <td className="px-4 py-2 border">
                  <Link
                    to={{
                      pathname: "/trade/report/level/user/pending",
                  ...
                  >
                    View App Wise Details
                  </Link>
                </td>
                <td className="px-4 py-2 border">
                  <Link
                    to={{
                      pathname: "/trade/report/level/user/wise/pending",
                      search: createSearchParams({
                        "roleId[]": item?.id.toString(), // Or pass multiple like: ["1", "2"]
                      }).toString(),
                    }}
...
export default LevelWisePending;
