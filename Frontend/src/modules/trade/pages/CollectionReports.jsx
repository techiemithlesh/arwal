...
//   );
  ...
  const summaryLink = useMemo(() => {
    ...
    return `/trade/report/payment/mode/summary?${params.toString()}`;
  }, [filters]);
  ...
  const fetchPaymentMode = async () => {
    try {
      const response = await axios.post(tradePaymentModeListApi);
  ...
  const renderRow = (item, index, currentPage, perPage) => (
    <tr key={item.id} className="hover:bg-gray-50">
      ...
      <td className="flex items-center gap-2 px-3 py-2 border">
        <Link
          to={`/trade/payment-receipt/${item.id}`}
          target="_blank"
  ...
  const renderFooter = (totals) => (
    <tr className="bg-gray-200 font-bold">
      ...
      <td colSpan={7} className="px-3 py-2 border border-white">{totals.totalCount}</td>
