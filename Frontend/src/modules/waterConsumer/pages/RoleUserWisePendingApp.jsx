...
import { waterLevelUserWisePendingApi } from "../../../api/endpoints";
import { exportToExcel, exportToPdf } from "../../../utils/exportUtils";
import { Link, createSearchParams, useSearchParams } from "react-router-dom";

function RoleUserWisePendingApp() {
  ...
  const fetchData = async () => {
    setIsFrozen(true);
    try {
      const res = await axios.post(waterLevelUserWisePendingApi, filters, {
          ...
          <tbody>
            {dataList.map((item, idx) => (
              <tr key={item.id || idx} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border">{item.totalApp}</td>
                <td className="px-4 py-2 border">{item.totalWard}</td>
                <td className="px-4 py-2 border">
                  <Link
                    to={{
                      pathname: "/water/consumer/report/level/user/pending",
...
export default RoleUserWisePendingApp
