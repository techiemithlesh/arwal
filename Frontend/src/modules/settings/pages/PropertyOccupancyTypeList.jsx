...
import PropertyOccupancyTypeModal from "../components/PropertyOccupancyTypeModal";
import {
  getOccupancyTypeListApi,
  getOccupancyTypeLockUnlockApi,
} from "../../../api/endpoints";
import { formatLocalDateTime } from "../../../utils/common"; // ✅ Added

function PropertyOccupancyTypeList() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Error fetching occupancy type list:", err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const headers = [
    { label: "#", key: "serial" },
    { label: "Occupancy Name", key: "occupancyName" },
    { label: "Multi Factor", key: "multFactor" },
  ...
  const renderRow = (item, index, currentPage, perPage) => (
    <tr key={item.id} className="hover:bg-gray-50">
      ...
      <td className="px-3 py-2 border">{item.occupancyName}</td>
      <td className="px-3 py-2 border">{item.multFactor}</td>
      <td className="px-3 py-2 border">
...
export default PropertyOccupancyTypeList;
