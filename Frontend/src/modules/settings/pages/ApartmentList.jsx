...
import { getApartmentListApi, getApartmentLockUnlockApi } from '../../../api/endpoints';
import { formatLocalDate } from '../../../utils/common';
import PermissionWrapper from '../../../components/common/PermissionWrapper';
import CommonTable from '../../../components/common/CommonTable';
import ImagePreview from '../../../components/common/ImagePreview';
import ApartmentAddEditModal from '../components/ApartmentAddEditModal';
...
function ApartmentList() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Error fetching construction type list:", err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const headers = [
    ...
    { label: "Apartment Code", key: "aptCode" },
    { label: "Name", key: "apartmentName" },
    { label: "Address", key: "apartmentAddress" },
    { label: "Has Water Harvesting", key: "isWaterHarvesting" },
    { label: "Harvesting Date", key: "waterHarvestingDate" },
    { label: "No Of Block", key: "noOfBlock" },
  ...
  const renderRow = (item, index, currentPage, perPage) => (
    <tr key={item.id} className="hover:bg-gray-50">
      ...
      <td className="px-3 py-2 border">{item.aptCode}</td>
      <td className="px-3 py-2 border">
        <img
            src={item?.apartmentImage}
            alt="apartmentImage"
            onClick={() => openPreviewModel(item?.apartmentImage)}
            className="inline-block ml-2 w-10 h-10 rounded-full object-cover border border-gray-300 cursor-pointer mr-3"
        />
        {item.apartmentName}
      </td>
      <td className="px-3 py-2 border">{item.apartmentAddress}</td>
      <td className="px-3 py-2 border">
        {item.isWaterHarvesting &&(
          <img
              src={item?.waterHarvestingImage}
              alt="waterHarvestingImage"
              onClick={() => openPreviewModel(item?.waterHarvestingImage)}
              className="inline-block ml-2 w-10 h-10 rounded-full object-cover border border-gray-300 cursor-pointer mr-3"
          />
        )}
        {item.isWaterHarvesting}
      </td>
      <td className="px-3 py-2 border">{formatLocalDate(item.waterHarvestingDate)}</td>
      <td className="px-3 py-2 border">{item.noOfBlock}</td>      
