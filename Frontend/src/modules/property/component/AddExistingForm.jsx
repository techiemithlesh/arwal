...
const AddExistingForm = ({
  ...
  const validateHoldingNo = async ()=>{
    setIsLoadingGable(true);
    try{
      const holdingNo = formData?.holdingNo;
      if(!holdingNo){
        return;
      }
      const response = await axios.post(validateHoldingUniqueApi,
        {holdingNo},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response?.data?.status) {
        ...
        toastMsg("This Holding No. Already Exists", "error");
        setErrors((prev) => ({
          ...prev,
          holdingNo: "This Holding No. Already Exists",
        }));
      }
  ...
  return (
    <div className="container-fluid">
      <form className="flex flex-col gap-4" onSubmit={handlePreviewFormData}>
          ...
          {[3, 4].includes(Number(formData?.propTypeMstrId)) && (
            <div className="">
              <label
                ...
                className="block font-medium text-sm"
              >
                Date of Possession / Purchase / Acquisition &nbsp;
          ...
          {formType=="Existing" &&(
            <div>
              ...
              <select 
                ...
                onChange={(e) => (e.target.value ? confirm("Are You Sure. Demand Paid Upto "+e.target.selectedOptions[0].text)  : true) && handleInputChange(e)} 
                className="block bg-white shadow-sm px-3 py-2 border border-gray-300 focus:border-indigo-500 rounded-md focus:outline-none focus:ring-indigo-500 w-full sm:text-xs"
              >
              ...
              {error?.demandPaidUpto && (
                <FormError name="demandPaidUpto" errors={error} />
              )}
            </div>
          )}
        ...
        {/* SWM */}
        {["New Assessment","Existing"].includes(formType) && formData?.propTypeMstrId != 4 && (
