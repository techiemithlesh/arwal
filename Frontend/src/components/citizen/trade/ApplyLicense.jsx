...
export default function ApplyLicense({ mstrData, formDetails }) {
  ...
  const [formData, setFormData] = useState(() => {
    const savedData = getWithExpiry("tradeConnectionFormData");
    if (savedData) {      
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.status) {
        ...
        setWithExpiry("tradeConnectionFormData", formData, 15);
        setWithExpiry("tradeMasterData", masterData, 15);
        setWithExpiry("tradeNewWardList", newWardList, 15);
        navigate("/citizen/trade/apply-license/preview");
      } else {
