...
const RecursiveMenu = ({ items, isCollapsed }) => {
    ...
    >
      {items.map((item) => (
        <li className="flex flex-col gap-y-1 w-full" key={item.title}>
          {item.link ? (
            // If item has a link → whole row is clickable
            <Link
              to={item.link}
              className={`flex items-center gap-2 p-2 rounded-lg transition-all duration-200 
                hover:bg-gray-100 dark:hover:bg-gray-700
                ${!isCollapsed ? "justify-between" : "justify-center"} 
                ${isActive(item.link) ? "bg-gray-200 dark:bg-gray-600 font-bold" : ""}
              `}
            >
            ...
            // If no link → it’s a toggle for submenu
            <div
              onClick={() => item.subItems && toggleItem(item.title)}
              className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all duration-200 
                hover:bg-gray-100 dark:hover:bg-gray-700
                ${!isCollapsed ? "justify-between" : "justify-center"} 
                ${openItems[item.title] ? "bg-gray-100 dark:bg-gray-700 font-bold" : ""}
              `}
            >
              <div className="flex items-center gap-3">
                {item.icon && <span className="text-base">{item.icon}</span>}
          ...
          {item.subItems && openItems[item.title] && !isCollapsed && (
            ...
            >
              <RecursiveMenu items={item.subItems} isCollapsed={isCollapsed} />
            </motion.div>
          )}
        </li>
      ))}
